...
class _ScriptedLLM:
    """MainBrain (run_tool_loop_structured) scriptat. NX-297: nu mai există `classify_json` de
    scriptat — triajul a fost șters, iar un fake care încă l-ar oferi ar ține în viață un contract
    mort."""

    model_agent = "model-de-test"

    def __init__(self, *, plan: dict | None = None, search: bool = True):
        self.plan = plan
        self.search = search
...
async def test_flag_off_first_reply_wins_byte_identical(monkeypatch):
    ...
    llm = _ScriptedLLM(plan=_plan_dict())
...
# NX-297: `test_flag_on_simple_route_served_by_brain_not_nano` a fost ȘTERS, nu mutat. Verifica
# faptul că un reply al triajului e demote-uit în semnal și că brain-ul răspunde în locul lui.
# Nano nu mai există, deci afirmația e adevărată prin CONSTRUCȚIE, iar un test care o repetă ar
# măsura absența unui modul — adică ar trece și dacă mecanismul de demotare s-ar strica. Demotarea
# rămâne testată pe stagiile care CHIAR mai pot răspunde (`faq_stage`, mai sus).
...
async def test_flag_on_not_ready_selects_current_live_via_real_selector(monkeypatch):
    ...
    await run_pipeline(ctx, PipelineDeps(conn=None, llm=llm), [agent_stage, fallback_stage])
