...
async def _cache_writeback(db: DbProvider, llm, business_id, locale, body, ctx) -> None:
    ...
    try:
        ...
        if not canonical:
            return
        # NX-181/183: scrie în namespace-ul versiunii prompt+envelope (simetric cu lookup-ul din
        # cache_stage) → vNext/v2 nu suprascriu intrări v1. Single source: cache_prompt_version.
