...
3. **Securitate — plasă RLS adăugată peste schema_v2** prin `docs/003_bot_runtime_role.sql`: rol `bot_runtime` fără bypassrls + politici `app.business_id` + `CHECK pg_column_size(state) < 8192`. `service_role` rămâne doar pentru migrări/joburi admin. Astfel principiul 7 (RLS ca plasă) e respectat. Workerul aplicației se conectează cu `bot_runtime`, NU cu service_role.
